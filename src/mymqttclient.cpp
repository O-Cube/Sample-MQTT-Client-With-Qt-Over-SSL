    ...
    , m_port( 0 )
{
    ...
    config.addCaCertificates( QString( "/root/Sample-MQTT-Client-With-Qt-Over-SSL/CA/ca.crt" ) );

    QByteArray keydata;
    QFile file( "/root/Sample-MQTT-Client-With-Qt-Over-SSL/CA/ca.key" );
    file.open( QIODeviceBase::ReadOnly | QIODeviceBase::Text );
    keydata = file.readAll();
    file.close();

    QSslKey key( keydata, QSsl::Rsa, QSsl::Pem, QSsl::PrivateKey, "oyandut" );
    config.setPrivateKey( key );
    connectToHostEncrypted( config );
